        ...
        <table class="table table-hover table-centered table-striped table-bordered text-center">
            <thead>
                <tr>
                    ...
                    <th class="border-gray-200">From Date</th>
                    <th class="border-gray-200">To Date</th>
            ...
            <tbody>
                ...
                @foreach ($promo_codes as $promo)
                    <tr>
                        ...
                        <td>
                            <p class="text-nowrap">{{ $promo->id }}.</p>
                        </td>

                        <td> <span class="badge bg-primary text-nowrap"
                            style="font-size: 16px">{{ $promo->title }}
                        </span>
                       </td>
                        ...
                        <td> <span class="badge bg-success text-nowrap"
                            style="font-size: 16px">{{ number_format($promo->value)  }} <span>{{ $promo->type =='amount'? ' EGP' :'%' }}</span>
                        </span>
                       </td>
                        ...
                        <td>
                            @if ($promo->from)
                                <span class="text-nowrap d-block h6"><i class="fas fa-calendar-week text-info"></i>
                                    {{ \Carbon\Carbon::parse($promo->from)->format('d/m/Y') }} </span>
                        ...
                        <td>
                            @if ($promo->to)
                                <span class="text-nowrap d-block h6"><i class="fas fa-calendar-week text-info"></i>
                                    {{ \Carbon\Carbon::parse($promo->to)->format('d/m/Y') }} </span>
                        ...
                        <td>
                            ...
                            <div class="d-flex  align-items-center justify-content-center flex-md-nowrap">
                                ...
                                <div class="">
                                    <div data-bs-toggle="tooltip" data-bs-placement="top" title="Show Details">
                                        <button class="btn btn-primary  m-1" data-bs-toggle="modal"
                                            data-bs-target="#modal-{{ $promo->id }}" class=""><span
                                ...
                                <div class="">
                                    @if (Auth::guard('admin')->user()->hasPermission('promo-codes-update'))
                                        <a class="btn btn-info m-1" class="dropdown-item"
                                            href="{{ route('admin.promo-codes.edit', $promo->id) }}" title="Edit"
                                ...
                                <div class="">
                                    @if ($promo->deleted_at)
                                        @if (Auth::guard('admin')->user()->hasPermission('promo-codes-update'))
                                            <form action="{{ route('admin.promo-codes.restore', $promo->id) }}" method="POST"
                                    ...
                                    @else
                                        @if (Auth::guard('admin')->user()->hasPermission('promo-codes-delete'))
                                            <form class="delete-btn m-0 p-0 "
                                                action="{{ route('admin.promo-codes.destroy', $promo->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        <div class="wrapper m-auto">
                                            ...
                                            <ul class="list-group list-group-flush">
                                                ...
                                                <li class="list-group-item">Added By :
                                                    ...
                                                    @if ($promo->admin)
                                                        <a class="btn btn-outline-primary"
                                                            href="{{ route('admin.admins.index', ['admin_id' => $promo->admin->id]) }}">{{ $promo->admin->name }}</a>
                                                    @else
                                                        --
                                                    @endif
                                                ...
                                                <li class="list-group-item">From Date : <i
                                                        class="fas fa-calendar-week text-danger"></i>
                                                    {{ \Carbon\Carbon::parse($promo->from)->format('d/m/Y') }}
                                                </li>
                                                <li class="list-group-item">To Date : <i
                                                    class="fas fa-calendar-week text-danger"></i>
                                                {{ \Carbon\Carbon::parse($promo->to)->format('d/m/Y') }}
                                            </li>
