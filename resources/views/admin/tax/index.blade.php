...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-2">
    ...
    <div class="btn-toolbar mb-2 mb-md-0">
        ...
        <div class="btn-group ms-2 ms-lg-3">
            <a href="{{ route('admin.taxes.export') }}" class="btn btn-outline-success d-inline-flex align-items-center">
                {{-- <svg class="icon icon-xs me-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg> --}}
            ...
            <tr>
                ...
                <td>
                    ...
                    <div class="m-auto" style="width:250px">
                        <p class="" dir="rtl">{{ Str::limit($tax->title_ar, 60) }}</p>
                        <p class="">{{ Str::limit($tax->title_en, 60) }}</p>
                    </div>
                </td>

                <td>
                    <span class="badge bg-info" style="font-size: 18px">{{ $tax->percentage }}%</span>
                ...
                <td>
                    @if ($tax->created_at)
                        <span class="text-nowrap d-block h6"><i class="fas fa-calendar-week text-info"></i>
                            {{ \Carbon\Carbon::parse($tax->created_at)->format('d/m/Y') }} </span>
                        <span class="text-nowrap d-block h6"><i class="fas fa-clock text-success"></i>
                            {{ \Carbon\Carbon::parse($tax->created_at)->format('h:i:s A') }} </span>
                    @else
                        ...
                    @endif
                        ...
                        <div class="">
                            <div data-bs-toggle="tooltip" data-bs-placement="top" title="Show Details">
                                <button class="btn btn-primary  m-1" data-bs-toggle="modal"
                                    data-bs-target="#modal-{{ $tax->id }}" class=""><span
                        ...
                        <div class="">
                            @if (Auth::guard('admin')->user()->hasPermission('taxes-update'))
                                <a class="btn btn-info m-1" class="dropdown-item"
                                    href="{{ route('admin.taxes.edit', $tax->id) }}" title="Edit"
                ...
                <div class="modal fade" id="modal-{{ $tax->id }}" tabindex="-1" role="dialog" aria-labelledby="modal-default" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                                ...
                                <div class="wrapper m-auto">
                                    ...
                                    <ul class="list-group list-group-flush">
                                        ...
                                        <li class="list-group-item">Added By :
                                            ...
                                            <a class="btn btn-outline-primary" href="{{ route('admin.admins.index' ,[ 'admin_id'=>$tax->admin->id ] )}}">{{ $tax->admin->name }}</a>
                                            @else
                                               --
                                            @endif
                                        ...
                                        <li class="list-group-item">Added Date :  <i class="fas fa-calendar-week text-info"></i> {{ \Carbon\Carbon::parse($tax->created_at)->format('d/m/Y')}}  </li>
                                        <li class="list-group-item">Added Time : <i class="fas fa-clock text-success"></i> {{ \Carbon\Carbon::parse($tax->created_at)->format('h:i:s A')}}</li>
                            ...
                            <div class="modal-footer">
                                ...
                                @if (Auth::guard('admin')->user()->hasPermission('taxes-update'))
                                <a class="btn btn-primary" href="{{ route('admin.taxes.edit', $tax->id) }}"><span class="fas fa-edit me-2"></span>Edit</a>
