...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-2">
    ...
    <div class="btn-toolbar mb-2 mb-md-0">
        ...
        <a href="{{ route('admin.countries.create') }}" class="btn btn-sm btn-gray-800 d-inline-flex align-items-center">
        ...
        <div class="btn-group ms-2 ms-lg-3">
            <a href="{{ route('admin.countries.export') }}" class="btn btn-outline-success d-inline-flex align-items-center">
                {{-- <svg class="icon icon-xs me-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg> --}}
    ...
    <table class="table table-hover table-centered table-striped table-bordered text-center">
        ...
        <tbody >
            ...
            <tr>
                ...
                <td>
                    <p class="text-nowrap">{{ $country->id }}.</p>
                </td>
                ...
                <td>
                    ...
                    <div class="m-auto" style="width:250px">
                        <p class="" dir="rtl">{{ Str::limit($country->title_ar, 60) }}</p>
                        <p class="">{{ Str::limit($country->title_en, 60) }}</p>
                    </div>
                </td>
                ...
                <td>
                    @if ($country->created_at)
                        <span class="text-nowrap d-block h6"><i class="fas fa-calendar-week text-info"></i>
                            {{ \Carbon\Carbon::parse($country->created_at)->format('d/m/Y') }} </span>
                        <span class="text-nowrap d-block h6"><i class="fas fa-clock text-success"></i>
                            {{ \Carbon\Carbon::parse($country->created_at)->format('h:i:s A') }} </span>
                    @else
                        ...
                    @endif
                        ...
                        <div class="">
                            <div data-bs-toggle="tooltip" data-bs-placement="top" title="Show Details">
                                <button class="btn btn-primary  m-1" data-bs-toggle="modal"
                                    data-bs-target="#modal-{{ $country->id }}" class=""><span
                        ...
                        <div class="">
                            @if (Auth::guard('admin')->user()->hasPermission('countries-update'))
                                <a class="btn btn-info m-1" class="dropdown-item"
                                    href="{{ route('admin.countries.edit', $country->id) }}" title="Edit"
            ...
            <div class="modal fade" id="modal-{{ $country->id }}" tabindex="-1" role="dialog" aria-labelledby="modal-default" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="wrapper m-auto">
                                ...
                                <ul class="list-group list-group-flush">
                                    <li class="list-group-item">Title In Arabic : {{ $country->title_ar }} </li>
                                    <li class="list-group-item">Title In English : {{ $country->title_en }} </li>
                                    <li class="list-group-item">Shortcut :  {{ $country->shortcut}}</li>
                                    <li class="list-group-item">Code :  {{ $country->code }}</li>
                                    <li class="list-group-item">Cites Count :  {{ $country->cities_count }}</li>

                                    <li class="list-group-item">Added By :

                                        @if ($country->admin)

                                        <a class="btn btn-outline-primary" href="{{ route('admin.admins.index' ,[ 'admin_id'=>$country->admin->id ] )}}">{{ $country->admin->name }}</a>
                                        @else
                                           --
                                        @endif
                                    ...
                                    <li class="list-group-item">Added Date :  <i class="fas fa-calendar-week text-info"></i> {{ \Carbon\Carbon::parse($country->created_at)->format('d/m/Y')}}  </li>
                                    <li class="list-group-item">Added Time : <i class="fas fa-clock text-success"></i> {{ \Carbon\Carbon::parse($country->created_at)->format('h:i:s A')}}</li>
                        ...
                        <div class="modal-footer">
                            ...
                            @if (Auth::guard('admin')->user()->hasPermission('countries-update'))
                            <a class="btn btn-primary" href="{{ route('admin.countries.edit', $country->id) }}"><span class="fas fa-edit me-2"></span>Edit</a>
